...
class SpanIgnoringSpanExporterTests {
	...
	void should_handle_span_when_not_yet_finished() {
		SpanIgnoringSpanExporter handler = new SpanIgnoringSpanExporter(beanFactory, new SleuthProperties(), spanExporter);
	...
	void should_not_handle_span_when_present_in_main_list_of_spans_to_skip() {
		...
		SpanIgnoringSpanExporter handler = new SpanIgnoringSpanExporter(beanFactory, sleuthProperties, spanExporter);
	...
	private void end(SpanIgnoringSpanExporter handler) {
		handler.end(null, namedSpan(), SpanHandler.Cause.FINISHED);
	}

	private SpanIgnoringSpanExporter handler(SleuthProperties sleuthProperties) {
		return new SpanIgnoringSpanExporter(beanFactory, sleuthProperties, spanExporter);
	}
