...
public class BraveSamplerAutoConfiguration {
	...
	static Sampler samplerFromProps(SamplerProperties config) {
		if (config.getProbability() != null) {
			return new BraveSampler(CountingSampler.create(config.getProbability()));
		}
		return new BraveSampler(brave.sampler.RateLimitingSampler.create(config.getRate()));
	}
	...
	protected static class RefreshScopedSamplerConfiguration {
		...
		private Sampler sampler(SamplerProperties config) {
			...
			// internally
			if (config.getProbability() != null) {
				return new BraveSampler(new ProbabilityBasedSampler(config));
			}
			return new BraveSampler(new RateLimitingSampler(config));
		}
