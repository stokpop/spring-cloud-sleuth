...
import org.springframework.cloud.bravebridge.BraveTracer;
...
@ConditionalOnProperty(value = "spring.sleuth.brave.enabled", matchIfMissing = true)
@AutoConfigureBefore(TraceAutoConfiguration.class)
@EnableConfigurationProperties(SleuthProperties.class)
@Import({ TraceBraveBaggageConfiguration.class, SamplerAutoConfiguration.class })
...
public class TraceBraveAutoConfiguration {
	...
	@Bean
	Span braveSpanCustomizer(SpanCustomizer spanCustomizer, Tracer tracer) {
		return new BraveOtelCurrentSpanCustomizer(spanCustomizer, tracer);
	}
	...
	SpanHandler spanIgnoringSpanHandler(SleuthProperties sleuthProperties) {
		return new BraveSpanIgnoringSpanHandler(sleuthProperties);
	}

}


class BraveOtelCurrentSpanCustomizer extends OtelSpanCustomizer {

	private final SpanCustomizer spanCustomizer;

	public BraveOtelCurrentSpanCustomizer(SpanCustomizer spanCustomizer, Tracer tracer) {
		super(tracer);
		this.spanCustomizer = spanCustomizer;
	}

	@Override
	public void setAttribute(String key, @javax.annotation.Nullable String value) {
		spanCustomizer.tag(key, value);
	}

	@Override
	public void setAttribute(String key, long value) {
		spanCustomizer.tag(key, String.valueOf(value));
	}
	...
	public void addEvent(String name) {
		spanCustomizer.annotate(name);
	}

	@Override
	public void addEvent(Event event) {
		spanCustomizer.annotate(event.getName());
	}
