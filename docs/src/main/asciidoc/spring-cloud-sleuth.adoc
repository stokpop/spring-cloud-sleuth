...
* `BraveTracer` - to get a start new spans ad-hoc
...
TIP: Spring Cloud Sleuth creates an instance of `BraveTracer` for you. In order to use it, you can autowire it.

=== Creating and finishing spans [[creating-and-finishing-spans]]

You can manually create spans by using the `BraveTracer`, as shown in the following example:
...
The `BraveTracer` object is fully managed by sleuth, so you rarely need to affect it. That said,
...
If you have OpenTracing on the classpath, we automatically register the OpenTracing `BraveTracer` bean.
...
* `TextMapPropagator.Setter<MessageHeaderAccessor>` - for writing headers to the message
* `TextMapPropagator.Getter<MessageHeaderAccessor>` - for reading headers from the message
